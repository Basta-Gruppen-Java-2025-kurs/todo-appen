...
import se.bastagruppen.todo_appen.exception.ResourceNotFoundException;
...
public class ToDoListCatalogService {
    ...
    public ToDoListCatalogResponseDto createCatalog(Long userId, String name) {
    ...
    public List<ToDoListCatalogResponseDto> getCatalogsForUser(Long userId) {
    ...
    public void deleteCatalog(Long id) {
        //TODO: only loged in owner of a catalog should be able to delete
        if(!catalogRepository.existsById(id)) {
            throw new ResourceNotFoundException("Catalog", id);
        }
        catalogRepository.deleteById(id);
    }

    private ToDoListCatalogResponseDto mapToResponse(ToDoListCatalog catalog) {
        return new ToDoListCatalogResponseDto(catalog.getId(), catalog.getName(), catalog.getUser().getId());
    }
}
